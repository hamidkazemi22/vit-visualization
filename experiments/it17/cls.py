...
class ViTFeatSelector(nn.Module):
    def __init__(self, sl: slice = None):
        super().__init__()
        self.sl = sl if sl is not None else slice(None, None)
...
class ViTCLSLayer(nn.Module):
    def __init__(self, vit: AugmentedModel, layer: int):
        super().__init__()
        self.pre = ViTPre(vit)
        self.cls_select = ViTFeatSelector(slice(0, 1))
        self.patches_select = ViTFeatSelector(slice(1, None))
        self.early = ViTTransformer(vit, slice(0, layer))
        self.deep = ViTTransformer(vit, slice(layer, None))
        self.head = ViTHead(vit, slice(0, 1))

    def forward(self, x: torch.tensor) -> torch.tensor:
        x = self.pre(x)
        cls = self.cls_select(x)
        patches = self.patches_select(x)
        cls = self.early(cls)
        patches = self.early(patches)
        both = torch.cat([cls, patches], dim=1)
        both = self.deep(both)
...
def main():
    ...
    model = ViTCLSLayer(model_base, layer)
    ...
    for i, (data) in enumerate(tqdm(loader)):
        ...
        if i % 100 == 0:
            print(name, f'TEAR_{network}_{layer}: \t{ac.value * 100} \t {ac5.value * 100} \t {ls.value}')

    with open('Acc.txt', 'a') as f:
        print(name, f'TEAR_{network}_{layer}: \t{ac.value * 100} \t {ac5.value * 100} \t {ls.value}', file=f)
        print(name, f'TEAR_{network}_{layer}: \t{ac.value * 100} \t {ac5.value * 100} \t {ls.value}')
