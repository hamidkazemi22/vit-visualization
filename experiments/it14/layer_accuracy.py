...
def main():
    ...
    model = nn.Sequential(ViTPre(model_base), ViTTransformer(model_base), ViTHead(model_base))
    ...
    for i, (data) in enumerate(tqdm(loader)):
        ...
        if i % 100 == 0:
            print(name, f'CLS_{network}: \t{ac.value * 100} \t {ac5.value * 100} \t {ls.value}')

    with open('Acc.txt', 'a') as f:
        print(name, f'CLS_{network}: \t{ac.value * 100} \t {ac5.value * 100} \t {ls.value}', file=f)
        print(name, f'CLS_{network}: \t{ac.value * 100} \t {ac5.value * 100} \t {ls.value}')
