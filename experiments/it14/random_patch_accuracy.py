...
def main():
    ...
    model = nn.Sequential(ViTPre(model_base), ViTTransformer(model_base), ViTHead(model_base, slice(120, 121)))
    ...
    for i, (data) in enumerate(tqdm(loader)):
        ...
        if i % 100 == 0:
            print(name, f'Patch120_{network}: \t{ac.value * 100} \t {ac5.value * 100} \t {ls.value}')

    with open('Acc.txt', 'a') as f:
        print(name, f'Patch120_{network}: \t{ac.value * 100} \t {ac5.value * 100} \t {ls.value}', file=f)
        print(name, f'Patch120_{network}: \t{ac.value * 100} \t {ac5.value * 100} \t {ls.value}')
