...
def main():
    ...
    saver = ExperimentSaver(f'GAUSL{layer}F{feature}_{tv}x{1.0}_LR{lr}_N{network}_M{method}_S{coef}_G{grid}',
                            save_id=True,
                            disk_saver=True)
    ...
    pre, post = torch.nn.Sequential(RepeatBatch(8), ColorJitter(8, shuffle_every=True),
                                    GaussianNoise(8, True, 0.5 * grid, 400), Tile(image_size // image_size),
                                    Jitter()), Clip()
